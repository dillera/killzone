 ...
 */
void display_close(void) { display_clear(); }
...
void display_show_welcome(const char *server_name) {
  ...
  /* Build full URL */
  if (SERVER_PORT == 80) {
    snprintf(url_buf, sizeof(url_buf), "  %s://%s", SERVER_PROTO, SERVER_HOST);
  } else {
    snprintf(url_buf, sizeof(url_buf), "  %s://%s:%d", SERVER_PROTO,
...
void display_draw_world(void) {
  ...
  for (i = 0; i < wall_count && i < MAX_WALLS; i++) {
    ...
    if (wall->x < DISPLAY_WIDTH && wall->y < DISPLAY_HEIGHT) {
      display_draw_char(wall->x, wall->y,
                        CHAR_WALL_CROSS); /* Use cross as temporary marker */
    }
  }
  ...
  for (y = 0; y < DISPLAY_HEIGHT; y++) {
    for (x = 0; x < DISPLAY_WIDTH; x++) {
      if (screen_buffer[y][x] == CHAR_WALL_CROSS) {
        ...
        uint8_t s = (y < DISPLAY_HEIGHT - 1 &&
                     screen_buffer[y + 1][x] == CHAR_WALL_CROSS);
        uint8_t e = (x < DISPLAY_WIDTH - 1 &&
                     screen_buffer[y][x + 1] == CHAR_WALL_CROSS);
        uint8_t w = (x > 0 && screen_buffer[y][x - 1] == CHAR_WALL_CROSS);

        if (n && s)
          screen_buffer[y][x] = CHAR_WALL;
        else if (e && w)
          screen_buffer[y][x] = CHAR_WALL_H;
        else if (s && e)
          screen_buffer[y][x] = CHAR_WALL_TL;
        else if (s && w)
          screen_buffer[y][x] = CHAR_WALL_TR;
        else if (n && e)
          screen_buffer[y][x] = CHAR_WALL_BL;
        else if (n && w)
          screen_buffer[y][x] = CHAR_WALL_BR;
      }
    }
  }
  ...
  if (local_player && local_player->x < DISPLAY_WIDTH &&
      local_player->y < DISPLAY_HEIGHT) {
...
void display_draw_status(const player_state_t *player) {
  ...
  printf("\nPlayer: %s | Pos: (%d,%d) | Health: %d | Status: %s\n", player->id,
         player->x, player->y, player->health, player->status);
}
...
void display_draw_status_bar(const char *player_name, uint8_t player_count,
  ...
  snprintf(line_buf, sizeof(line_buf), "%s P:%d %s", player_name, player_count,
           connection_status);
  ...
  /* cputsxy(0, 22, "----------------------------------------"); */
  {
    ...
    if (w_count > 40) {
      ...
      snprintf(line_buf, sizeof(line_buf), "W[0]:(%d,%d) Total:%d", walls[0].x,
               walls[0].y, w_count);
      cputsxy(0, 22, line_buf);
    } else {
